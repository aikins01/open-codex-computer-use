...
### Changes Overview
**Scope:** `OpenComputerUseKit` macOS screenshot capture, README, architecture docs, release notes
...
- **[Runtime]**: runtime 进程读取环境配置后，`WindowCapture` 使用同一份配置控制 ScreenCaptureKit 超时、PNG 长边上限、编码后字节预算和降采样比例下限。
